...
class OrcidService:
    ...
    def __init__(self, orcid_base_url=None):
        self.orcid_base_url = orcid_base_url or self.ORCID_BASE_URL

    def build_auth_url(self, user_id, return_url=None):
    ...
    def connect_orcid_account(self, user, token_data):
    ...
    def get_redirect_url(self, error=None, return_url=None):
    ...
    def _get_orcid_app(self):
        return SocialApp.objects.get(provider=OrcidProvider.id)

    def _is_valid_redirect_url(self, url):
        if not url:
            return False
        parsed = urlparse(url)
    ...
    def _encode_signed_value(self, value):
        return signing.dumps(value)

    def _decode_signed_value(self, signed_value, max_age=None):
        try:
